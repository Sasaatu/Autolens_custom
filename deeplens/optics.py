...
class Lensgroup():
    ...
    def refocus_inf(self):
        ...
        for i in range(num_wave):
            ray = self.sample_parallel_2D(R=self.surfaces[0].r, M=M, wavelength=self.wave[i])
            ray, _, _ = self.trace(ray)
    ...
    def refocus(self, depth=DEPTH):
        ...
        for i in range(num_wave):
            ...
            ray = Ray(o, d, normalized=False, wavelength=self.wave[i], device=self.device)
            ray, _, _ = self.trace(ray)
    ...
    @torch.no_grad()
    def analysis(self, save_name='./test', multi_plot=False, draw_layout=False, draw_spot_diagram=False, plot_invalid=True, zmx_format=True, point_depth=None, lens_title=None):
    ...
    def plot_setup2D_with_trace(self, filename, views=[0], M=9, depth=None, entrance_pupil=True, zmx_format=False, plot_invalid=True, multi_plot=False, lens_title=None):
        ...
        if multi_plot:
            ...
            for i, wavelength in enumerate(self.wave):
                ax = axs[i]
        ...
        # ==> Plot RGB in one figure
            ...
            for i, view in enumerate(views):
                if i == 0:
                    ...
                    idx = math.floor(len(self.wave)/2)
                elif i == 2:
                    idx = -1

                if depth is None:
                    ray = self.sample_parallel_2D(R, self.wave[idx], view=view, M=M, entrance_pupil=entrance_pupil)
                else:
                    ray = self.sample_point_source_2D(depth=depth, view=view, M=M, entrance_pupil=entrance_pupil, wavelength=self.wave[idx])
    ...
    def loss_infocus(self, M=31):
        ...
        focz = self.d_sensor
        loss = []
        for wv in [self.wave[0], self.wave[-1]]:
    ...
    def analysis_rms(self, depth=DEPTH, ref=True):
        ...
        rms_max_on = max(rms_on_axis)
        rms_max_off = max(rms_off_axis)
        rms_radius = rms_mean.sqrt()
        rms_radius_on_axis = rms_max_on.sqrt()
        rms_radius_off_axis = rms_max_off.sqrt()
    ...
    def refine(self, lrs=[5e-4, 1e-4, 0.1, 1e-4], decay=0.1, iterations=2000, test_per_iter=100, num_source=9, num_ray=256, depth=DEPTH, shape_control=True, centroid=False, importance_sampling=False, result_dir='./results', save_global=True, save_steps=True):
        ...
        for i in range(iterations+1):
            ...
            # =========================================
            loss_rms = []
            for j, wv in enumerate(self.wave):
                # => Ray tracing
